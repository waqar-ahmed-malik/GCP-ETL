...
class readfromoracle(beam.DoFn): 
      def process(self, context):
          ...
          for chunk in pd.read_sql(query, cnx, coerce_float=True, params=None, parse_dates=None, columns=None,chunksize=50000):
                 chunk.apply(lambda x: x.replace('\r', ' ').replace('\n', ' ') if isinstance(x, str) or isinstance(x, str) else x).to_gbq(product_config['datasetname']+"."+targettable, env_config['projectid'],if_exists='append')
...
def run():
    ...
    try:
        ...
        (dummy_env | 'Processing' >>  beam.ParDo(readfromoracle()))
        p=pcoll.run()
        p.wait_until_finish()
    except:
