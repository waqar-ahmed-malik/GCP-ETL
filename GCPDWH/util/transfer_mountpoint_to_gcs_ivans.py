...
class connecttosftp(beam.DoFn):
    ...
    def process(self,
        ...
        with pysftp.Connection(host=product_config['ftpserver'], username=product_config['ftpuser'],password=str(base64.b64decode(database_password.encode('utf-8')), 'utf-8'), cnopts = cnopts) as sftp:
            ...
            for loc in filelist:
                ...
                os.system('gsutil -m mv /tmp/* '+'gs://'+product_config['bucket']+'/ivans/current/')
...
def main(args_config,args_productconfig,args_env):
    ...
    jobname="copy-files-from-"+product_config['productname']+"-"+batchdate
