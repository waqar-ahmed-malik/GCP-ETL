...
This template load delimited files from the storage buckets into their corresponding landing tables using dataflow pipelines.
...
@author: MWG Data Team

Arguments Required :    
Eg :

-- 'python /home/airflow/gcs/data/GCPDWH/util/load_delimited_file_to_bigquery_landing.py --config config.properties --productconfig travelware.properties --env "Environment values like dev, QA and Prod" --input '+ filename + ' --separator "delimiter values" --stripheader 1 --stripdelim 0 --addaudit 1 --output "Output Table Name" --writeDeposition WRITE_APPEND')
...
#                     '--subnetwork',env_config['subnetwork'],
    ...
    try:
        ...
        valid_rows, error_rows = (readlines | 'Validate File Data' >> beam.Map(rowvalidator).with_outputs('error_rows',main='valid_rows'))
        clean_rows = valid_rows | 'Cleanse File Data' >> beam.Map(rowextractor)
        clean_rows | 'WriteBQ File Data'    >> beam.io.Write(
...
def main(config, productconfig, env, input,  separator, stripheader, stripdelim, addaudit,  output,writeDeposition ):
    ...
    #fileid=gssourcebucket.split('/')[4].split('_')[2].split('.')[0]
    fileid=gssourcebucket.split('/')[4].split('.')[0]
    print(fileid)
 #   fileid=gssourcebucket
    ...
    print(tablefields)
    jobname = lower("load-" + product_config['productname']+"-to-"+ product_config['datasetname'] +"-"+fileid)
