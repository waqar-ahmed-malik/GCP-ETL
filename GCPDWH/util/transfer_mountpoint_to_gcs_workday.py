        ...
        with pysftp.Connection(host=env_config['ftpserver'],port=22, username=env_config['ftpuser'],password=str(base64.b64decode(database_password.encode('utf-8')), 'utf-8'), cnopts = cnopts) as sftp:
            ...
            for loc in filelist:
                ...
                for f in sftp.listdir_attr():
                    ...
                    filename_reverse=reverse(f.filename)

                    filename_part=filename_reverse[4:]

                    filename_correct=reverse(filename_part)

                    os.system('mv /tmp/'+f.filename+' /tmp/'+filename_correct)

                    date_suffix=time.strftime("%m.%d.%Y")

                    os.system('mv /tmp/'+filename_correct+' /tmp/'+filename_correct+'.'+date_suffix+'.csv')



                os.system('gsutil -m mv /tmp/* '+'gs://'+product_config['bucket']+'/current/')
