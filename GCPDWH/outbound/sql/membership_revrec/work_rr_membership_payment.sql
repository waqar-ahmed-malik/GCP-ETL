...
CASE WHEN GLACT_KEY=11 then TERM_EFFECTIVE_DT
     ...
     ELSE TERM_EFFECTIVE_DT END AS ACC_PERIOD_EFFECTIVE_DT,
CASE WHEN REC_COUNT=1 AND GLACT_TYPE='FEES' THEN DATE_SUB(TERM_EXPIRATION_DT, INTERVAL 1 DAY)
     WHEN  GLACT_TYPE='FEES' THEN  DATE_SUB(DATE_ADD(TERM_EXPIRATION_DT, INTERVAL 7 YEAR), INTERVAL 1 DAY)
     ELSE DATE_SUB(TERM_EXPIRATION_DT, INTERVAL 1 DAY) END AS ACC_PERIOD_EXPIRATION_DT,
...
'P' AS MEMBERSHIP_PAYMENT_TRAN_TYPE_CD,
...
WHERE (MEMBERSHIP_PAYMENT_TRAN_TYPE_CD  IN ('P') OR ( MEMBERSHIP_PAYMENT_TYPE_KEY = 12 AND MEMBERSHIP_PAYMENT_TRAN_TYPE_CD = 'A'))
...
from OPERATIONAL.CONNECTSUITE_MEMBERSHIP_PAYMENT 
WHERE (MEMBERSHIP_PAYMENT_TRAN_TYPE_CD  IN ('P') OR ( MEMBERSHIP_PAYMENT_TYPE_KEY = 12 AND MEMBERSHIP_PAYMENT_TRAN_TYPE_CD = 'A'))) MSP_EXTRA
...
ON MSP.MEMBERSHIP_PAYMENT_KEY=CJE.MEMBERSHIP_PAYMENT_KEY))
