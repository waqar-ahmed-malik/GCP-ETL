...
def run():
    try:        
        ...
        df_detail = df_detail.rename(columns={'GROUP1': 'GROUP'})
        #Reading audit data from audit SQL in pandas Dataframe
        df_audit_stg= pandas.read_gbq(readfileasstring(sql_query_folder+audit_sql_file), project_id=env_config['projectid'], dialect='standard') 
        ...
        #Creating all the subelements as rows and columns from DStaframe
            for (col,c) in zip(df_detail.columns,list(df_detail)):
                if (c=='LINE_ITEM' or c=='DESCRIPTION' or c=='AMOUNT') and hierarchy_flag=='1':   
                    if flg==1: 
                        rows_2=ET.SubElement(rows, "INVOICE_LINE_ITEMS")     
                        rows_3=ET.SubElement(rows_2, "INVOICE_LINE_ITEM")                                    
                    ET.SubElement(rows_3, c).text = str(df_detail[col][i])
                    flg=0
                else:    
...
def main(args_config,args_productconfig,args_env,args_detailsqlfile,args_auditsqlfile,args_filename,args_bucketname,args_foldername,args_hierarchyflag):
    ...
    global hierarchy_flag
    hierarchy_flag = args_hierarchyflag
    global jobrunid
    jobrunid=os.getpid()
