SELECT
 ...
 TRIM(CONVERT(CHAR,
  ...
  -------------------------------    
   ...
   ELSE 'N'
    END AS DECEASED_IND  ,
    TRIM(CONVERT(CHAR,CIC.ID_VALUE,50)) AS ONLINE_SIGNUP_IND
FROM (
  SELECT
    MBR.MBR_KY,
    MBR.MBRS_ID,
...
ON
  ...
  FROM (
    SELECT
      UPPER(MADR_BSC_AD) AS MADR_BSC_AD,
      UPPER(MADR_SUPL_AD) AS MADR_SUPL_AD,
      UPPER(MADR_CTY_NM) AS MADR_CTY_NM,
      UPPER(MADR_ST_PROV_CD) AS MADR_ST_PROV_CD,
      UPPER(MADR_ZIP_CD) AS MADR_ZIP_CD,
      MBRS_KY,
      CASE
...
LEFT OUTER JOIN   
 MRM_SAM_PRD.DBO.CUSTOMER_ID_SOURCE CIS 
 ON 
 CIS.CUSTOMER_ID  = CUST.CUSTOMERID AND CIS.SOURCE_SYSTEM= 'WEB'
 LEFT OUTER JOIN   
 MRM_SAM_PRD.DBO.CUSTOMER_ID_COLUMN CIC
 ON CIS.ID_SOURCE_ID = CIC.ID_SOURCE_ID
