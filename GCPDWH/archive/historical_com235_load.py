...
def main(args_config,args_productconfig,args_env):
    ...
    for blob in bucket.list_blobs(prefix='current/'):
        ...
        if  'COM23500' in filename and 'PGP.dec'in filename:
            #print blob
            filename=blob.name.split('/')[1]
            print(filename)
            ...
            #print fileid
            loadtables="python "+cwd+"\\"+"load_com235_to_bigquery_landing.py " + "--config config.properties --productconfig lifeinsurance.properties --env prod  --input "+ file +" --separator , --stripheader 1 --stripdelim 0  --addaudit 1   --output WORK_LIFE_COM235   --writeDeposition WRITE_APPEND"
    ...
    logging.info("COM235 Loading process completed..")
