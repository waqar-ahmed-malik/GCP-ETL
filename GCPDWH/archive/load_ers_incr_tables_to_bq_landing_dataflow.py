...
def run():
    ...
    try:
        ...
        work_d2k_audit=(dummy_env | 'WORK D2K AUDIT' >>  beam.ParDo(readfromoracle(),landing_ers_d2k_audit,'WORK_ERS_STG_APD_D2K_AUDIT'))
        d2k_audit=(work_d2k_audit | 'D2K AUDIT' >>  beam.ParDo(runbqsql(),merge_ers_stage_d2k_audit))
      
        work_apd_remb_detail=(dummy_env | 'WORK APD REMB DETAIL' >>  beam.ParDo(readfromoracle(),landing_ers_apd_remb_detail,'WORK_ERS_STG_APD_REMB_DETAIL'))
        apd_remb_detail=(work_apd_remb_detail | 'APD REMB DETAIL' >>  beam.ParDo(runbqsql(),merge_ers_stage_apd_remb_detail))
    
        work_apd_svc_fac_adj_item=(dummy_env | 'WORK APD SVC FAC ADJ ITEM' >>  beam.ParDo(readfromoracle(),landing_ers_apd_svc_fac_adj_item,'WORK_ERS_STG_APD_SVC_FAC_ADJ_ITEM'))
        apd_svc_fac_adj_item=(work_apd_svc_fac_adj_item | 'APD SVC FAC ADJ ITEM' >>  beam.ParDo(runbqsql(),merge_ers_stage_apd_svc_fac_adj_item))
      
        work_cad_svc_vehicle=(dummy_env | 'WORK CAD SVC VEHICLE' >>  beam.ParDo(readfromoracle(),landing_ers_cad_svc_vehicle,'WORK_ERS_STG_CAD_SVC_VEHICLE'))
        cad_svc_vehicle=(work_cad_svc_vehicle | 'CAD SVC VEHICLE' >>  beam.ParDo(runbqsql(),merge_ers_stage_cad_svc_vehicle))
    
        work_hist_aar_referral=(dummy_env | 'WORK HIST AAR REFERRAL' >>  beam.ParDo(readfromoracle(),landing_ers_hist_aar_referral,'WORK_ERS_STG_HIST_AAR_REFERRAL'))
        hist_aar_referral=(work_hist_aar_referral | 'HIST AAR REFERRAL' >>  beam.ParDo(runbqsql(),merge_ers_stage_hist_aar_referral))
       
        work_inq_ent_periods=(dummy_env | 'WORK INQ ENTITLEMENT PERIODS' >>  beam.ParDo(readfromoracle(),landing_ers_inq_ent_periods,'WORK_ERS_STG_INQ_ENTITLEMENT_PERIODS'))
        inq_ent_periods=(work_inq_ent_periods | 'INQ ENTITLEMENT PERIODS' >>  beam.ParDo(runbqsql(),merge_ers_stage_inq_ent_periods))
  
        work_cad_tow_dest=(dummy_env | 'WORK CAD TOW DEST' >>  beam.ParDo(readfromoracle(),landing_ers_cad_tow_dest,'WORK_ERS_STG_CAD_TOW_DEST'))
        cad_tow_dest=(work_cad_tow_dest | 'CAD TOW DEST' >>  beam.ParDo(runbqsql(),merge_ers_stage_cad_tow_dest))
 
        work_payment=(dummy_env | 'WORK PAYMENT' >>  beam.ParDo(readfromoracle(),landing_ers_stage_payment,'WORK_ERS_STG_PAYMENT'))
        payment=(work_payment | 'PAYMENT' >>  beam.ParDo(runbqsql(),merge_ers_stage_payment))
 
        work_reports_data=(dummy_env | 'WORK REPORTS_DATA' >>  beam.ParDo(readfromoracle(),landing_ers_stage_reports_data,'WORK_ERS_STG_REPORTS_DATA'))
        reports_data=(work_reports_data | 'REPORTS_DATA' >>  beam.ParDo(runbqsql(),merge_ers_stage_reports_data))
         
        work_start_grp=(dummy_env | 'WORK STAT_GRP' >>  beam.ParDo(readfromoracle(),landing_ers_stage_stat_grp,'WORK_ERS_STG_STAT_GRP'))
        start_grp=(work_start_grp | 'STAT_GRP' >>  beam.ParDo(runbqsql(),merge_ers_stage_stat_grp))
         
        work_truck_gps_hist=(dummy_env | 'WORK TRUCK_GPS_HIST' >>  beam.ParDo(readfromoracle(),landing_ers_stage_truck_gps_hist,'WORK_ERS_STG_TRUCK_GPS_HIST'))
        truck_gps_hist=(work_truck_gps_hist | 'TRUCK_GPS_HIST' >>  beam.ParDo(runbqsql(),merge_ers_stage_truck_gps_hist))
        
        work_apd_reimbursement=(dummy_env | 'WORK APD REIMBURSEMENT' >>  beam.ParDo(readfromoracle(),landing_ers_apd_reimbursement,'WORK_ERS_STG_APD_REIMBURSEMENT'))
        apd_reimbursement=(work_apd_reimbursement | 'ERS APD REIMBURSEMENT' >>  beam.ParDo(runbqsql(),merge_ers_stage_apd_reimbursement))
    
        work_call_adj=(dummy_env | 'WORK HIST CALL ADJ' >>  beam.ParDo(readfromoracle(),landing_ers_hist_call_adj,'WORK_ERS_STG_HIST_CALL_ADJ'))
        call_adj=(work_call_adj | 'HIST CALL ADJ' >>  beam.ParDo(runbqsql(),merge_ers_stage_call_adj))
    
        work_cad_truck_session_stats=(dummy_env | 'WORK CAD TRUCK SESSION STATS' >>  beam.ParDo(readfromoracle(),landing_ers_cad_session_stats,'WORK_ERS_STG_CAD_TRUCK_SESSION_STATS'))
        cad_truck_sess_stats=(work_cad_truck_session_stats | 'CAD TRUCK SESSION STATS' >>  beam.ParDo(runbqsql(),merge_ers_stage_cad_session_stats))
   
        work_cad_sc_call_comment=(dummy_env | 'WORK CAD SC CALL COMMENT' >>  beam.ParDo(readfromoracle(),landing_ers_cad_sc_call_comment,'WORK_ERS_STG_CAD_SC_CALL_COMMENT'))
        cad_sc_call_comment=(work_cad_sc_call_comment | 'CAD SC CALL COMMENT' >>  beam.ParDo(runbqsql(),merge_ers_stage_cad_sc_call_comment))
  
        work_spot_grid=(dummy_env | 'WORK SPOT GRID' >>  beam.ParDo(readfromoracle(),landing_ers_spot_grid,'WORK_ERS_STG_SPOT_GRID'))
        spot_grid=(work_spot_grid | 'SPOT GRID' >>  beam.ParDo(runbqsql(),merge_ers_stage_spot_grid))
...
def main(args_config,args_productconfig,args_env,args_connectionprefix,args_incrementaldate):
    ...
    incrementaldate_d2k=date_string_d2k
 
    blob_svc_fac_itm= Blob('paramfiles/result_FACIL_INT_ID.csv', bucket)
    print("blob name is "+blob_svc_fac_itm.name)
    downloaded_blob_svc_fac_itm = blob_svc_fac_itm.download_as_string()
    string_svc_fac_itm =downloaded_blob_svc_fac_itm[14:33]
    new_sentence = string_svc_fac_itm[:]
    for letter in string_svc_fac_itm:
        if not letter.isalnum():
            incremental_svc_fac_itm = new_sentence.replace(letter, ' ')
    ...
    incrementaldate_cad_svc_veh=date_string_cad_svc_veh
    ...
    incrementaldate_aar_referral=date_string_aar_referral
    ...
    incrementaldate_enti_period=date_string_enti_period
    ...
    incrementaldate_tow_dest=date_string_tow_dest
    ...
    incrementaldate_payment=date_string_payment
    ...
    incrementaldate_reports=date_string_reports
    ...
    incrementaldate_gpshist =date_string_gpshist
    ...
    incrementaldate_statgrp =date_string_statgrp
    ...
    incrementaldate_reim =date_string_reim
    ...
    incrementaldate_adj =date_string_adj
    ...
    incrementaldate_sc_call =date_string_sc_call
    ...
    incrementaldate_session_stats =date_string_session_stats   
    ...
    incrementaldate_spot_grid =date_string_spot_grid 
                                                        
    print("incrementadldate_d2k is")
    print(incrementaldate_d2k)
    print("incrementaldate_svc_fac_itm is")
    print(incremental_svc_fac_itm)   
