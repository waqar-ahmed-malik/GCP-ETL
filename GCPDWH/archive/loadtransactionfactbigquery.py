...
1. Reads data from underlying landing tables i.e. TST, Comments and Invoices
2. This module takes Input Query(sql File) and Target tables as arguments. 
...
def run(): 
    try:
        exec(compile(open('D://PROJECTS//AAA//cloud_sdk//GCPDWH//util//readtableschema.py').read(), 'D://PROJECTS//AAA//cloud_sdk//GCPDWH//util//readtableschema.py', 'exec'), globals())
        ...
        inpQuery=readfileasstring(inputquery)
...
def main(args_config,args_productconfig,args_env,args_targettable,args_inputquery):
    global env
    env = args_env
    global config
    config= args_config
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--inputquery',
        required=True,
        help= ('Input Query to read data from underlying Big Query Tables')
        )
