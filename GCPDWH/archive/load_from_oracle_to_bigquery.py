...
def run():
    ...
    query = readfileasstring(sqlfile).replace('jobrunid',str(jobrunid)).replace('jobname', jobname)
    ...
    for chunk in pd.read_sql(query, cnx, coerce_float=True, params=None, parse_dates=None, columns=None,chunksize=10000):
