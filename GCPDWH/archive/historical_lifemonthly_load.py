...
def main(args_config,args_productconfig,args_env):
    ...
    for blob in bucket.list_blobs(prefix='current/'):
        ...
        if  'csaprmth' in filename and 'PGP.dec'in filename:
            #print blob
            filename=blob.name.split('/')[1]
            print(filename)
            file="gs://"+product_config['bucket']+"/current/"+filename
            print(file)
            #fileid=file.split('/')[4].split('.')[2][:6]
            #print fileid
            loadtables="python "+cwd+"\\"+"load_lifemonthly_to_bigquery_landing.py " + "--config config.properties --productconfig lifeinsurance.properties --env prod  --input "+ file +" --separator , --stripheader 0 --stripdelim 0  --addaudit 1   --output WORK_LIFE_INS_CSAPRDLY_MONTHLY   --writeDeposition WRITE_APPEND"
    ...
    logging.info("Life monthly Loading process completed..")
