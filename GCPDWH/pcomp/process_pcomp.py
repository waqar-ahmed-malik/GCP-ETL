...
def main(args_config,args_productconfig,args_env):
    ...
    for blob in bucket.list_blobs(prefix='current/'):
        ...
        if  'PCSI.GBPD.CPCS.COMPUP.'+current_date.strftime("%m.%d.%Y")+'.CSV' in filename:
      ...
      #  elif  'PAS_E_EXGPAS_PRCMPS_4002_D.'+now.strftime("%m.%d.%Y") in filename:
        ...
        elif  'QryHomeIncentive' in filename and current_date.strftime("%m%d%y") in filename:
        ...
        elif  'PCSI.GBPD.CPCS.XSDLYSTA.'+current_date.strftime("%m.%d.%Y")+'.CSV' in filename:
