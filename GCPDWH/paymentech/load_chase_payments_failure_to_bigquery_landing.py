...
#                    '--requirements_file', env_config['requirements_file'],
    ...
    try:
        ...
        read18D = (readlines | 'Read RPDE0018D RecType' >> beam.ParDo(filter_recordtype(),recordtype='18D'))

        valid_rows, error_rows = (read18D | 'Validate Payment Failures' >> beam.Map(rowvalidator).with_outputs('error_rows',main='valid_rows'))

        clean_rows = valid_rows | 'Cleanse Payment Failures' >> beam.Map(rowextractor)

        clean_rows | 'WriteBQ Payment Failures'    >> beam.io.Write(
...
def main(config, productconfig, env, input,  separator, stripheader, stripdelim, addaudit,  output,writeDeposition ):
    ...
    jobname = "load-" + product_config['productname']+"-to-"+ product_config['datasetname'] +"-"+"failures"+fileid.replace('.csv','')
