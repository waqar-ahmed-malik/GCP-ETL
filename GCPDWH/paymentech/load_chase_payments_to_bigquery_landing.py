...
#                    '--requirements_file', env_config['requirements_file'],
    ...
    try:
        ...
        readlines = pcoll | 'Read chase payment Records' >> beam.io.ReadFromText(gssourcebucket,skip_header_lines = remove_header_rows) 

        valid_rows, error_rows = (readlines | 'Validate chase payment' >> beam.Map(rowvalidator).with_outputs('error_rows',main='valid_rows'))

        clean_rows = valid_rows | 'Cleanse chase payment' >> beam.Map(rowextractor)

        clean_rows | 'WriteBQ chase payment'    >> beam.io.Write(
...
def main(config, productconfig, env, input,  separator, stripheader, stripdelim, addaudit,  output,writeDeposition ):
    ...
    print("gssourcebucket"+gssourcebucket)

    vfileid= "20"+gssourcebucket.split('/')[4].split('.')[2]
    ...
    jobname = "load-" + product_config['productname']+"-to-"+ product_config['datasetname'] +"-"+"payments"+fileid.replace('.csv','')
