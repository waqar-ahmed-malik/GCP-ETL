...
class movetoarchive(beam.DoFn):

    def process(self,context):

        '''This module moves the files from current folder of the bucket to the archive folder upon completion of Loading process'''

        os.system('gsutil -m mv '+bucket_folders+'/current/* '+bucket_folders+'/archive/')
...
#                     '--subnetwork',env_config['subnetwork'],
    ...
    try:
        ...
        valid_rows, error_rows = (readlines | 'Validate Claims' >> beam.Map(rowvalidator).with_outputs('error_rows',main='valid_rows'))

        clean_rows = valid_rows | 'Cleanse Claims' >> beam.Map(rowextractor)

        clean_rows | 'WriteBQ Claims'    >> beam.io.Write(
        ...
        #flatten=((load_rows,error_handler) | 'Flatten MSI' >> beam.Flatten())

        #flatten_globally= flatten | 'Dummy : Flatten Globally MSI' >> beam.CombineGlobally(any)

        

        #archive_files=(flatten_globally | 'Archiving Files..' >>  beam.ParDo(movetoarchive()))
...
def main(config, productconfig, env, input,  separator, stripheader, stripdelim, addaudit,  output,writeDeposition ):
    ...
    input_split = gssourcebucket.split('/')

    bucket_folder = [input_split[0],input_split[1],input_split[2]]

    bucket_folders=''.join(bucket_folder)

    print("bucket folder is "+bucket_folders)
    ...
    #fileid=gssourcebucket.split('/')[4].split('_')[2] +gssourcebucket.split('/')[4].split('_')[3]+gssourcebucket.split('/')[4].split('_')[4]

    

    #fileid_kiosk=gssourcebucket.split('/')[4].split('_')[2].split('.')[1] +gssourcebucket.split('/')[4].split('_')[2].split('.')[2]+gssourcebucket.split('/')[4].split('_')[2].split('.')[3]

    #fileid=gssourcebucket.split('/')[4].split('_')[1] +gssourcebucket.split('/')[4].split('_')[2]+gssourcebucket.split('/')[4].split('_')[3].split('.')[0]

    #fileid_napa=gssourcebucket.split('/')[4].split('.')[1] +gssourcebucket.split('/')[4].split('.')[2]+gssourcebucket.split('/')[4].split('.')[3].split('.')[0]

    fileid=gssourcebucket
    ...
    jobname = "load-" + product_config['productname']+"-to-"+ product_config['datasetname'] +"-"+fileid.replace('.csv','').replace('/','').replace(':','').replace('_','-').replace('.','').replace(' ','')
