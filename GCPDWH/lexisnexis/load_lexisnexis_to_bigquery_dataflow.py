...
#           database_password=env_config[connectionprefix+'_database_password'].decode('base64')          database_password = env_config[connectionprefix + '_database_password']          database_password = str(base64.b64decode(database_password.encode('utf-8')), 'utf-8')
          ...
          for chunk in pandas.read_sql(query.replace("v_database_name", database_database), cnx, coerce_float=True, params=None, parse_dates=None, columns=None,chunksize=100000):
...
def run():
    ...
    try:
        ...
        (dummy_env | 'WORK LEXIS NEXIS' >>  beam.ParDo(readfromssql(),landing_lexis_nexis ,landingtablename)
 
        | 'LEXIS NEXIS' >>  beam.ParDo(runbqsql(),lexis_nexis))
...
def main(args_config,args_productconfig,args_env,args_connectionprefix,args_incrementaldate,args_partition,args_landingtablename):
    ...
    global jobname
    jobname="load-"+product_config['productname']+'-'+partition+'-'+time.strftime("%Y%m%d")
    logging.info('Job Name is %s',jobname)
    
    global landing_lexis_nexis
    global lexis_nexis
...
#     delete_data_direct_focus = readfileasstring(cwd + '/sql/delete_direct_focus_data.sql')
    ...
    landing_lexis_nexis = readfileasstring(cwd + '/sql/work_lexisnexis.sql').replace('v_modvalue',partition)

#     landing_direct_focus_contact_email = readfileasstring(cwd + '/sql/work_direct_focus_contact_email.sql').replace('v_incr_date',incrementaldate)
#     landing_direct_focus_et_email_bounces = readfileasstring(cwd + '/sql/work_direct_focus_et_email_bounces.sql').replace('v_incr_date',incrementaldate)
#     landing_direct_focus_et_subscriber_attr = readfileasstring(cwd + '/sql/work_direct_focus_et_subscriber_attr.sql').replace('v_incr_date',incrementaldate)

    lexis_nexis = readfileasstring(cwd + '/sql/insert_lexis_nexis.sql').replace('jobrunid',str(jobrunid)).replace('v_tablename',landingtablename)
#     df_contact_email = readfileasstring(cwd + '/sql/df_contact_email.sql').replace('jobrunid',str(jobrunid)).replace('jobname', jobname)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--partition',
        required=True,
        help= ('work table number for sql query')
        )
    
    parser.add_argument(
        '--landingtablename',
        required=True,
        help= ('work table name')
        )
