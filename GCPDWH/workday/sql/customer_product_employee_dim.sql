WITH base AS (
  SELECT 
    ...
    CASE WHEN Worker_P_C_10_Digit IS NOT NULL THEN LPAD(Worker_P_C_10_Digit,10,'0') ELSE Worker_P_C_10_Digit END AS PAS_AGENT_ID,
  ...
  SELECT 
    ...
    COALESCE(DATE_SUB(LEAD(file_date) OVER (PARTITION BY ORG_CODE ORDER BY file_date), INTERVAL 1 DAY),'2199-12-31') AS end_date,
    MANAGER,
    CASE 
  ...
  FROM LANDING.WD_WORKER WHERE length(empid) = 6
), costcenter AS (
  SELECT 
    Cost_Center_CD,
    Cost_Center_Desc,
...
Select EMP.* except(ALL_TERMINATION_DT)
from (
SELECT 
  aa.EMPLOYEE_ID,
  aa.ENT_ID,
  ...
  aa.JOB_CD,
  ee.JOB_CD_DESC,
  ee.JOB_CD_EFFECTIVE_DT,
  cc.SUPERVISOR_EMPLOYEE_ID,
  aa.ORGANIZATION_CD,
  aa.WORKER_AVAYA_ID,
  bb.SUPERVISOR_HIERARCHY_LEVEL,
  bb.LEVEL_01,
  bb.LEVEL_02,
  bb.LEVEL_03,
  bb.LEVEL_04,
  ...
  aa.MD5_VALUE,
  aa.FILE_ID,
  max(aa.TERMINATION_DT) OVER termwindow AS ALL_TERMINATION_DT
FROM base aa
...
WINDOW termwindow AS (PARTITION BY aa.EMPLOYEE_ID, aa.HIRE_DT)
) EMP
WHERE EMP.ROW_END_DT <=  CASE WHEN ALL_TERMINATION_DT IS NULL THEN '9999-12-31' ELSE ALL_TERMINATION_DT END
