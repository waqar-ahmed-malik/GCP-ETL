INSERT INTO CUSTOMER_PRODUCT.INSURANCE_TRANSACTION_FACT(INSURANCE_TRANSACTION_ID	,	
TRANSACTION_DT			,
TRANSACTION_EFFECTIVE_DT,	
TRANSACTION_ACCOUNTING_DT,
TRANSACTION_TERM			, 
TRANSACTION_TYPE			, 
TRANSACTION_AMT			,
TRANSACTION_CD			,
...
CASE WHEN T1.TRANSACTION_EFFECTIVE_DT IS NOT NULL THEN T1.TRANSACTION_EFFECTIVE_DT
	 ELSE T1.POLICY_EFFECTIVE_DT END TRANSACTION_EFFECTIVE_DT,  
CASE  WHEN T1.TRANSACTION_DT < CASE WHEN T1.TRANSACTION_EFFECTIVE_DT IS NULL THEN T1.POLICY_EFFECTIVE_DT 
                               ELSE T1.TRANSACTION_EFFECTIVE_DT END THEN CASE WHEN T1.TRANSACTION_EFFECTIVE_DT IS NULL THEN T1.POLICY_EFFECTIVE_DT ELSE T1.TRANSACTION_EFFECTIVE_DT END
ELSE T1.TRANSACTION_DT  END  AS TRANSACTION_ACCOUNTING_DT,  
T1.TRANSACTION_TYPE AS TRANSACTION_TERM, 
T1.PROCESS_TYPE AS TRANSACTION_TYPE	, 
T1.TRANSACTION_AMT,
T1.TRANSACTION_CD,
...
CASE WHEN T1.SOURCE_SYSTEM IN ('Exigen Pup', 'Exigen Home', 'Exigen Auto') AND TRIM(T1.POLICY_PREFIX) IS NOT NULL  THEN CONCAT(TRIM(T1.POLICY_PREFIX), TRIM(T1.POLICY_NUM))
     WHEN TRIM(T1.SOURCE_SYSTEM) = 'CEA' THEN NULL
...
--W.LEGAL_FIRST_NM AS COMPENSATION_EMPLOYEE_FIRST_NM,
--W.LEGAL_LAST_NM AS COMPENSATION_EMPLOYEE_LAST_NM,
...
CASE WHEN LZD.LOCATION_ID IS NOT NULL THEN LZD.LOCATION_ID
     WHEN LZD1.LOCATION_ID IS NOT NULL THEN LZD1.LOCATION_ID
	 WHEN (TRIM(T1.SALES_REP_ID) in ('000','00','0', '0.0', '') OR TRIM(T1.SALES_REP_ID) IS NULL) AND TRIM(T1.PAS_AGENT_ID) IS NULL THEN '999'
	 WHEN TRIM(E1.EMPLOYEE_ID) IS NOT NULL THEN E1.LOCATION_ID
	 ELSE TRIM(E2.LOCATION_ID) END AS REVENUE_LOCATION_ID, 
...
T1.PRODUCT_SUB_CATEGORY AS PRODUCT_SUBCATEGORY, 
...
LEFT OUTER JOIN `CUSTOMER_PRODUCT.EMPLOYEE_DIM` E1 ON TRIM(T1.SALES_REP_ID) = TRIM(E1.INSURANCE_REP_ID) AND T1.TRANSACTION_DT BETWEEN E1.ROW_START_DT AND  E1.ROW_END_DT
--LEFT OUTER JOIN `CUSTOMER_PRODUCT.EMPLOYEE_DIM` E2 ON TRIM(T1.PAS_AGENT_ID) = TRIM(E2.PAS_AGENT_ID) AND T1.TRANSACTION_DT BETWEEN E2.ROW_START_DT AND  E2.ROW_END_DT
LEFT OUTER JOIN `CUSTOMER_PRODUCT.EMPLOYEE_DIM` E2 ON LPAD(TRIM(T1.PAS_AGENT_ID),10, '0') = LPAD(TRIM(E2.PAS_AGENT_ID),10, '0') AND E2.ACTIVE_FLG = 'Y'
--LEFT JOIN CUSTOMERS.EMPLOYEE_PROFILE_DIM W
--ON  coalesce(E1.EMPLOYEE_ID,E2.EMPLOYEE_ID) = W.EMPLOYEE_ID AND W.ACTIVE_FLG='Y'
LEFT OUTER JOIN `CUSTOMER_PRODUCT.LOCATION_DIM` B1  ON TRIM(coalesce(E1.LOCATION_ID, E2.LOCATION_ID )) = TRIM(B1.LOCATION_ID) AND B1.ACTIVE_FLG = 'Y'
LEFT OUTER JOIN `CUSTOMER_PRODUCT.LOCATION_ZIP_DIM` LZD ON TRIM(SUBSTR(T1.RESIDENTIAL_ZIP_CD,1,5)) =CAST(LZD.ZIP_CD as STRING)
LEFT OUTER JOIN `CUSTOMER_PRODUCT.LOCATION_ZIP_DIM` LZD1 ON TRIM(SUBSTR(T1.MAILING_ZIP_CD,1,5)) =CAST(LZD1.ZIP_CD as STRING)
