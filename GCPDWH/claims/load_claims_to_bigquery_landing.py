...
--config "config.properties" --productconfig "claims.properties" --env "dev"  --input "gs://dw-prod-claims/current/CLAIMS_NCNU_20160101_080040.txt.01.01.2016" --separator "|"  --stripheader "1" --stripdelim "0"  --addaudit "1"   --output "WORK_INSURANCE_CLAIMS"   --writeDeposition "WRITE_TRUNCATE"  
...
def main(config, productconfig, env, input,  separator, stripheader, stripdelim, addaudit,  output,writeDeposition ):
    ...
    fileid=gssourcebucket.split('/')[4].split('_')[2] 
