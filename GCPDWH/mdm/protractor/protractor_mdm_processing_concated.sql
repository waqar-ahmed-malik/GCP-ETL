...
(select SOURCE_KEY1 FROM LANDING.WORK_MDM_COR_CUSTOMER C, CUSTOMERS.MDM_CUSTOMER_DIM D
...
WHERE GOLDEN_MATCH_KEY in (select SOURCE_KEY1 from
  LANDING.WORK_MDM_COR_CUSTOMER C, (SELECT   *  FROM CUSTOMERS.MDM_CUSTOMER_DIM
...
UPDATE LANDING.WORK_MDM_COR_CUSTOMER C
...
FROM (
  ...
  FROM (SELECT * FROM LANDING.WORK_MDM_COR_CUSTOMER WHERE CUSTOMER_MDM_KEY IS NOT NULL AND MERGE_FLAG='Golden')  ) B
...
FROM (
  ...
  FROM
    LANDING.WORK_MDM_COR_CUSTOMER C
...
AND COALESCE(C.BIRTH_DT,CURRENT_DATE())=COALESCE(D.BIRTH_DT,CURRENT_DATE())
AND COALESCE(C.PRIMARY_ADDRESS_LINE1,'~')=COALESCE(D.ADDRESS_LINE1,'~')
AND COALESCE(C.PRIMARY_ADDRESS_LINE2,'~')=COALESCE(D.ADDRESS_LINE2,'~') 
AND COALESCE(C.PRIMARY_CITY,'~')=COALESCE(D.CITY,'~') 
AND COALESCE(C.PRIMARY_STATE,'~')=COALESCE(D.STATE,'~')
...
SET C.CUSTOMER_MDM_KEY = D.CUSTOMER_MDM_KEY , MDM_MATCH_GROUP = 'RULES'
 FROM (SELECT * FROM LANDING.WORK_MDM_COR_CUSTOMER WHERE MERGE_FLAG='Golden') D
 WHERE C.CUSTOMER_MDM_KEY IS NULL
 AND C.MERGE_FLAG <> 'Golden' 
...
--STEP 8: UPDATE MD5_KEY's in WORK_MDM_COR_CUSTOMER.


UPDATE LANDING.WORK_MDM_COR_CUSTOMER
