...
SELECT
    ...
    RESIDENTIAL_ZIP,
        ...
        SOURCE_CREATE_DATE,
    CURRENT_DATETIME()
FROM LANDING.WORK_MDM_SMART_HOME
WHERE GOLDEN_MATCH_KEY in 
(select SOURCE_KEY1 FROM LANDING.WORK_MDM_SMART_HOME C, CUSTOMERS.MDM_CUSTOMER_DIM D
...
DELETE FROM LANDING.WORK_MDM_SMART_HOME
WHERE GOLDEN_MATCH_KEY in 
(select SOURCE_KEY1 FROM LANDING.WORK_MDM_SMART_HOME C, CUSTOMERS.MDM_CUSTOMER_DIM D
WHERE  C.CUSTOMER_MDM_KEY IS NULL
AND C.MERGE_FLAG='Golden'
...
WHERE GOLDEN_MATCH_KEY in  (select SOURCE_KEY1 from
  LANDING.WORK_MDM_SMART_HOME C, (SELECT   *  FROM CUSTOMERS.MDM_CUSTOMER_DIM
...
UPDATE LANDING.WORK_MDM_SMART_HOME C
...
FROM (
  ...
  FROM (SELECT * FROM LANDING.WORK_MDM_SMART_HOME WHERE CUSTOMER_MDM_KEY IS NOT NULL AND MERGE_FLAG='Golden')  ) B
...
SET C.CUSTOMER_MDM_KEY = D.CUSTOMER_MDM_KEY , MDM_MATCH_GROUP = 'RULES'
 FROM (SELECT * FROM LANDING.WORK_MDM_SMART_HOME WHERE MERGE_FLAG='Golden') D
 WHERE C.CUSTOMER_MDM_KEY IS NULL
 AND C.MERGE_FLAG <> 'Golden' 
...
--STEP 8: UPDATE MD5_KEY's in WORK_MDM_SMART_HOME.


UPDATE LANDING.WORK_MDM_SMART_HOME
