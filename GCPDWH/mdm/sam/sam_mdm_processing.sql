...
--STEP 1: Find CUSTOMER_MDM_KEY on member_num_9 and Set it the work table

UPDATE LANDING.WORK_MDM_SAM A
SET A.CUSTOMER_MDM_KEY = B.CUSTOMER_MDM_KEY , MDM_MATCH_GROUP = 'SOURCE_KEY2'
FROM CUSTOMER_PRODUCT.MDM_CUSTOMER_BRIDGE B 
WHERE A.CUSTOMER_MDM_KEY IS NULL
AND B.SOURCE_SYSTEM_CD = 'MEMBERSHIP'
AND A.SOURCE_KEY2 = SUBSTR(B.SOURCE_KEY1,7,9);
...
(select SOURCE_KEY1 FROM LANDING.WORK_MDM_SAM C, CUSTOMERS.MDM_CUSTOMER_DIM D
...
WHERE GOLDEN_MATCH_KEY in (select SOURCE_KEY1 from
  LANDING.WORK_MDM_SAM C, (SELECT   *  FROM CUSTOMERS.MDM_CUSTOMER_DIM
...
FROM (
  ...
  FROM (SELECT * FROM LANDING.WORK_MDM_SAM WHERE CUSTOMER_MDM_KEY IS NOT NULL AND MERGE_FLAG='Golden')  ) B
...
SET C.CUSTOMER_MDM_KEY = D.CUSTOMER_MDM_KEY , MDM_MATCH_GROUP = 'RULES'
 FROM (SELECT * FROM LANDING.WORK_MDM_SAM WHERE MERGE_FLAG='Golden') D
 WHERE C.CUSTOMER_MDM_KEY IS NULL
 AND C.MERGE_FLAG <> 'Golden' 
...
--STEP 8: UPDATE MD5_KEY's in WORK_MDM_SAM.


UPDATE LANDING.WORK_MDM_SAM
