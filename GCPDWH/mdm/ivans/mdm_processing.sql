...
AND COALESCE(A.SOURCE_KEY2,'~') = COALESCE(B.SOURCE_KEY2,'~');
...
SELECT
    ...
    CURRENT_DATETIME()
FROM LANDING.WORK_MDM_IVANS_IE
WHERE GOLDEN_MATCH_KEY in 
(select SOURCE_KEY3 FROM LANDING.WORK_MDM_IVANS_IE C, CUSTOMERS.MDM_CUSTOMER_DIM D
...
DELETE FROM LANDING.WORK_MDM_IVANS_IE
WHERE GOLDEN_MATCH_KEY in 
(select SOURCE_KEY3 FROM LANDING.WORK_MDM_IVANS_IE C, CUSTOMERS.MDM_CUSTOMER_DIM D
WHERE  C.CUSTOMER_MDM_KEY IS NULL
AND C.MERGE_FLAG='Golden'
...
UPDATE LANDING.WORK_MDM_IVANS_IE C
...
FROM (
  ...
  FROM (SELECT * FROM LANDING.WORK_MDM_IVANS_IE WHERE CUSTOMER_MDM_KEY IS NOT NULL AND MERGE_FLAG='Golden')  ) B
...
FROM (
  ...
  FROM
    LANDING.WORK_MDM_IVANS_IE C
  WHERE CUSTOMER_MDM_KEY IS NULL AND C.MERGE_FLAG='Golden');  
...
SET C.CUSTOMER_MDM_KEY = D.CUSTOMER_MDM_KEY , MDM_MATCH_GROUP = 'RULES'
 FROM (SELECT * FROM LANDING.WORK_MDM_IVANS_IE WHERE MERGE_FLAG='Golden') D
 WHERE C.CUSTOMER_MDM_KEY IS NULL
 AND C.MERGE_FLAG <> 'Golden' 
 AND C. GOLDEN_MATCH_KEY = D.SOURCE_KEY3;
...
--STEP 9: UPDATE MD5_KEY's in WORK_MDM_IVANS_IE.

UPDATE LANDING.WORK_MDM_IVANS_IE
...
, COALESCE(CAST(SOURCE_KEY2_DESC AS STRING),' ')
...
--STEP 10: INSERT CUSTOMERS CONTACT DATA INTO WORK_MDM_IVANS_IE TABLE.
...
, SOURCE_KEY2_DESC
,'HOME' AS TYPE
, RESIDENTIAL_ADDRESS_LINE1 AS  ADDRESS_LINE1
, RESIDENTIAL_ADDRESS_LINE2 AS  ADDRESS_LINE2
, RESIDENTIAL_CITY AS  CITY
, RESIDENTIAL_STATE AS  STATE
, RESIDENTIAL_ZIP AS  ZIP_CD
, SOURCE_SYSTEM_CD
...
ROW_NUMBER() OVER (PARTITION BY  SOURCE_KEY1, SOURCE_KEY2,TYPE  ORDER BY ROW_START_DT DESC ) DUP_CHECK 
FROM CUSTOMERS.MDM_CUSTOMER_CONTACT_DIM WHERE ACTIVE_FLG = 'Y' AND SOURCE_SYSTEM_CD like 'IVANS%') 
...
AND COALESCE(DIM.SOURCE_KEY2,'~') = COALESCE(TEMP.SOURCE_KEY2,'~')	   
