...
--STEP 1: Find CUSTOMER_MDM_KEY on member_num and Set it the work table
...
SELECT
    ...
    SOURCE_CREATE_DATE,
    CURRENT_DATETIME()
FROM LANDING.WORK_MDM_MEMBERSHIP
WHERE GOLDEN_MATCH_KEY in 
(select SOURCE_KEY1 FROM LANDING.WORK_MDM_MEMBERSHIP C, CUSTOMERS.MDM_CUSTOMER_DIM D
...
DELETE FROM LANDING.WORK_MDM_MEMBERSHIP
WHERE GOLDEN_MATCH_KEY in 
(select SOURCE_KEY1 FROM LANDING.WORK_MDM_MEMBERSHIP C, CUSTOMERS.MDM_CUSTOMER_DIM D
WHERE  C.CUSTOMER_MDM_KEY IS NULL
AND C.MERGE_FLAG='Golden'
...
WHERE GOLDEN_MATCH_KEY in (select SOURCE_KEY1 from
  LANDING.WORK_MDM_MEMBERSHIP C, (SELECT   *  FROM CUSTOMERS.MDM_CUSTOMER_DIM
...
UPDATE LANDING.WORK_MDM_MEMBERSHIP C
...
SET
  ...
  A.PREFFERRED_NM = CASE
    WHEN B.PREFFERRED_NM IS NULL THEN A.PREFFERRED_NM
    ELSE B.PREFFERRED_NM END,
  A.MARITAL_STATUS = CASE
    WHEN B.MARITAL_STATUS IS NULL THEN A.MARITAL_STATUS
    ELSE B.MARITAL_STATUS END,
  A.DECEASED_IND = CASE
    WHEN B.DECEASED_IND IS NULL THEN A.DECEASED_IND
    ELSE B.DECEASED_IND END,
  A.BIRTH_DT=CASE
...
FROM (
  ...
  FROM (SELECT * FROM LANDING.WORK_MDM_MEMBERSHIP WHERE CUSTOMER_MDM_KEY IS NOT NULL AND MERGE_FLAG='Golden')  ) B
...
--STEP 7: UPDATE CUSTOMER_MDM_KEY ON NON GOLDEN CUSTOMERS IN WORK TABLE.
	
UPDATE LANDING.WORK_MDM_MEMBERSHIP C
SET C.CUSTOMER_MDM_KEY = D.CUSTOMER_MDM_KEY , MDM_MATCH_GROUP = 'RULES'
 FROM (SELECT * FROM LANDING.WORK_MDM_MEMBERSHIP WHERE MERGE_FLAG='Golden') D
 WHERE C.CUSTOMER_MDM_KEY IS NULL
 AND C.MERGE_FLAG <> 'Golden' 
 AND C.GOLDEN_MATCH_KEY = D.SOURCE_KEY1;

--STEP 7.2: UPDATE TERM_EXPIRATION_DT IN  BRIDGE TABLE FOR THE CUSTOMERS MATCHED ON MEMBER_NUM.

UPDATE CUSTOMER_PRODUCT.MDM_CUSTOMER_BRIDGE D
SET D.SOURCE_KEY2 = C.SOURCE_KEY2
FROM LANDING.WORK_MDM_MEMBERSHIP C
WHERE C.SOURCE_SYSTEM_CD = D.SOURCE_SYSTEM_CD
AND D.CUSTOMER_MDM_KEY=C.CUSTOMER_MDM_KEY
AND D.SOURCE_KEY1 = C.SOURCE_KEY1
AND MDM_MATCH_GROUP = 'SOURCE_KEY1'
AND D.CUSTOMER_MDM_KEY IS NOT NULL;


--STEP8: INSERT NEW/MATCHED CUSTOMERS DATA INTO BRIDGE TABLE.
...
--STEP 9: UPDATE MD5_KEY's in WORK_MDM_MEMBERSHIP.

UPDATE LANDING.WORK_MDM_MEMBERSHIP
...
MAILING_MD5_KEY=MD5(CONCAT(  COALESCE(CAST(SOURCE_KEY1 AS STRING),' ')
, COALESCE(CAST(SOURCE_KEY1_DESC AS STRING),' ')
, COALESCE(CAST(SOURCE_SYSTEM_CD AS STRING),' ')
, COALESCE(CAST('MAILING' AS STRING),' ')
...
OTHER_MD5_KEY= MD5(CONCAT(  COALESCE(CAST(SOURCE_KEY1 AS STRING),' ')
, COALESCE(CAST(SOURCE_KEY1_DESC AS STRING),' ')
, COALESCE(CAST(SOURCE_SYSTEM_CD AS STRING),' ')
, COALESCE(CAST('OTHER' AS STRING),' ')
, COALESCE(CAST(OTHER_ADDRESS_LINE1 AS STRING),' ')
, COALESCE(CAST(OTHER_ADDRESS_LINE2 AS STRING),' ')
, COALESCE(CAST(OTHER_CITY AS STRING),' ')
, COALESCE(CAST(OTHER_STATE AS STRING),' ')
, COALESCE(CAST(OTHER_ZIP	 AS STRING),' ')
, COALESCE(CAST(OTHER_EMAIL_ADDRESS AS STRING),' ')	
, COALESCE(CAST(OTHER_PHONE_NUM AS STRING),' '))) 
WHERE TRUE;

--STEP 10: INSERT CUSTOMERS CONTACT DATA INTO MDM_CUSTOMER_CONTACT_DIM TABLE.
...
FROM LANDING.WORK_MDM_MEMBERSHIP
WHERE PRIMARY_ADDRESS_LINE1 IS NOT NULL OR PRIMARY_EMAIL_ADDRESS IS NOT NULL OR HOME_TELEPHONE_NUM IS NOT NULL
UNION ALL
SELECT 
CUSTOMER_MDM_KEY
,SOURCE_KEY1
...
,SOURCE_SYSTEM_CD
,'MAILING' AS TYPE
, BILLING_ADDRESS_LINE1 AS  ADDRESS_LINE1
, BILLING_ADDRESS_LINE2 AS  ADDRESS_LINE2
, BILLING_CITY AS  CITY
, BILLING_STATE AS  STATE
, BILLING_ZIP AS  ZIP_CD
, NULL AS EMAIL_ADDRESS
, WORK_PHONE_NUM AS PHONE_NUM
, MAILING_MD5_KEY AS MD5_KEY
FROM LANDING.WORK_MDM_MEMBERSHIP
WHERE BILLING_ADDRESS_LINE1 IS NOT NULL OR WORK_PHONE_NUM IS NOT NULL
UNION ALL
SELECT 
CUSTOMER_MDM_KEY
,SOURCE_KEY1
...
,SOURCE_SYSTEM_CD
,'OTHER' AS TYPE
, OTHER_ADDRESS_LINE1 AS  ADDRESS_LINE1
, OTHER_ADDRESS_LINE2 AS  ADDRESS_LINE2
, OTHER_CITY AS  CITY
, OTHER_STATE AS  STATE
, OTHER_ZIP AS  ZIP_CD
, OTHER_EMAIL_ADDRESS AS EMAIL_ADDRESS
, OTHER_PHONE_NUM AS PHONE_NUM
, OTHER_MD5_KEY AS MD5_KEY
FROM LANDING.WORK_MDM_MEMBERSHIP
WHERE OTHER_ADDRESS_LINE1 IS NOT NULL OR OTHER_EMAIL_ADDRESS IS NOT NULL OR OTHER_PHONE_NUM IS NOT NULL) S
WHERE S.MD5_KEY NOT IN ( SELECT MD5_KEY FROM CUSTOMERS.MDM_CUSTOMER_CONTACT_DIM WHERE ACTIVE_FLG='Y');

--STEP 11: TYPE 2 UPDATE FOR MDM_CUSTOMER_CONTACT_DIM TABLE.
...
ROW_NUMBER() OVER (PARTITION BY  SOURCE_KEY1,TYPE  ORDER BY ROW_START_DT DESC ) DUP_CHECK 
FROM CUSTOMERS.MDM_CUSTOMER_CONTACT_DIM WHERE 
