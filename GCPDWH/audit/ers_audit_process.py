...
class readfromoracle(beam.DoFn): 
      ...
      def process(self, context, inpquery,targettable):
          ...
          for chunk in pd.read_sql(query, cnx, coerce_float=True, params=None, parse_dates=None, columns=None,chunksize=100):
...
def run():
    ...
    try:
        ...
        (dummy_env | 'DELETE WORK ERS AUDIT' >>  beam.ParDo(runbqsql(),delete_work_ers_audit) 

                             | 'INSERT WORK ERS AUDIT' >>  beam.ParDo(readfromoracle(),insert_work_ers_audit,'WORK_ERS_AUDIT')

                             | 'INSERT ERS AUDIT' >>  beam.ParDo(runbqsql(),insert_ers_audit)

                             | 'UPDATE ERS AUDIT' >>  beam.ParDo(runbqsql(),update_ers_audit))
...
def main(args_config,args_productconfig,args_env,args_connectionprefix):
    ...
    global delete_work_ers_audit          

    global insert_work_ers_audit

    global insert_ers_audit

    global update_ers_audit



    delete_work_ers_audit = readfileasstring(cwd+'/ers/delete_work_audit.sql')               

    insert_work_ers_audit = readfileasstring(cwd+'/ers/work_audit.sql')    

    insert_ers_audit = readfileasstring(cwd+'/ers/insert_audit.sql')  

    update_ers_audit = readfileasstring(cwd+'/ers/update_audit.sql')  
