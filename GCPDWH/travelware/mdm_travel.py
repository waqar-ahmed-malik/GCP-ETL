...
if cwd.find("\\") > 0:
    ...
    mdmpath = cwd[:folderup]+"\\mdm\\travel\\"
    
else:
    folderup = cwd.rfind("/")
    utilpath = cwd[:folderup]+"/util/"
    mdmpath = cwd[:folderup]+"/mdm/travel/"
...
def main(args_config,args_productconfig,args_env):
    ...
    work_sql_file=readfileasstring(mdmpath+"WORK_MDM_TRAVEL.sql").split(';')
    main_sql_file=readfileasstring(mdmpath+"mdm_processing.sql").split(';')
    for inputsql in work_sql_file:
        run_query(inputsql) 
    for inputsql in main_sql_file:
        run_query(inputsql) 
