...
def run():
    ...
    try:
        ...
        | 'DELETE WORK TABLES' >>  beam.ParDo(runbqsqloop(),delete_data_direct_focus)
        | 'WORK INDIVIDUAL' >>  beam.ParDo(readfromssql(),direct_focus_work_individual ,'WORK_INDIVIDUAL')
        | 'WORK INDIVIDUAL MD5' >>  beam.ParDo(runbqsql(),direct_focus_work_individual_md5)
        | 'WORK INDIVIDUAL ARCHIVE_DT' >> beam.ParDo(runbqsql(),direct_focus_individual_arch_dt_update))
        
        (dummy_env | 'WORK MDM LEXISNEXIS_INDIVIDUAL' >>  beam.ParDo(runbqsqloop(),load_mdm_stg_sql) 
        |  'MDM LEXISNEXIS INDIVIDUAL' >>  beam.ParDo(runbqsqloop(),load_mdm_processing_sql))
        
        (dummy_env | 'DIRECTFOCUS INDIVIDUAL DIM' >>  beam.ParDo(runbqsql(),direct_focus_individual_dim)
        | 'ROW DATE AND FLAG UPDATE' >>  beam.ParDo(runbqsql(),direct_focus_individual_dim_flag_update))
...
def main(args_config,args_productconfig,args_env,args_connectionprefix,args_incrementaldate):
    ...
    global jobname
    jobname="load-individual-"+product_config['productname']+'-'+time.strftime("%Y%m%d")
    logging.info('Job Name is %s',jobname)
    
    global direct_focus_work_individual
    global direct_focus_work_individual_md5
    global direct_focus_individual_dim
    global direct_focus_individual_dim_flag_update
    global delete_data_direct_focus
    global direct_focus_individual_arch_dt_update
    
    delete_data_direct_focus = readfileasstring(cwd + '/sql/Incremental/delete_work_individual.sql')
    
    direct_focus_work_individual = readfileasstring(cwd + '/sql/Incremental/work_individual.sql').replace('v_incr_date',incrementaldate)
    direct_focus_work_individual_md5 = readfileasstring(cwd + '/sql/Incremental/insert_work_individual_md5.sql')
    direct_focus_individual_arch_dt_update = readfileasstring(cwd + '/sql/Incremental/update_direct_focus_individual_arch_dt.sql') 
     
    direct_focus_individual_dim = readfileasstring(cwd + '/sql/Incremental/insert_direct_focus_individual_dim.sql').replace('jobrunid',str(jobrunid))
    direct_focus_individual_dim_flag_update = readfileasstring(cwd + '/sql/Incremental/direct_focus_individual_rowdate_update.sql')
    
    global load_mdm_stg_sql
    global load_mdm_processing_sql
    
    load_mdm_stg_sql=readfileasstring(mdmpath+"/lexisnexis/load_mdm_stg.sql")
    load_mdm_processing_sql=readfileasstring(mdmpath+"/lexisnexis/mdm_processing.sql")
