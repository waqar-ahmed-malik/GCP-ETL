...
def run():
    ...
    try:
        ...
        | 'DELETE WORK TABLES' >>  beam.ParDo(runbqsqloop(),delete_work_direct_focus_engage_journey_history)

        | 'WORK ENGAGE JOURNEY HISTORY' >>  beam.ParDo(readfromssql(),work_direct_focus_engage_journey_history,'WORK_DF_ENGAGE_JOURNEY_HISTORY')
...
def main(args_config,args_productconfig,args_env,args_connectionprefix,args_incrementaldate):
    ...
    jobname="load-engage-journey-history-"+product_config['productname']+'-'+time.strftime("%Y%m%d")
    ...
    global delete_work_direct_focus_engage_journey_history

    

    delete_work_direct_focus_engage_journey_history = readfileasstring(cwd + '/sql/Weekday/delete_work_df_engage_journey_history.sql')

    

    work_direct_focus_engage_journey_history = readfileasstring(cwd + '/sql/Weekday/insert_work_df_engage_journey_history.sql').replace('v_incr_date',incrementaldate)

     

    direct_focus_engage_journey_history = readfileasstring(cwd + '/sql/Weekday/insert_direct_focus_engage_journey_history.sql').replace('jobrunid',str(jobrunid))
