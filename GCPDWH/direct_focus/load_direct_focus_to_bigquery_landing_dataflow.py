...
def run():
    ...
    try:
        ...
        truncate_tables = (dummy_env | 'TRUNCATE WORK TABLES' >>  beam.ParDo(runbqsqloop(),delete_data_direct_focus))
#         landing_df_communication = (truncate_tables | 'WORK DIRECT FOCUS COMMUNICATION' >>  beam.ParDo(readfromssql(),landing_direct_focus_communication ,'WORK_DIRECT_FOCUS_COMMUNICATION'))
        landing_df_et_email_bounces  = (truncate_tables | 'WORK DIRECT FOCUS ET EMAIL BOUNCES' >>  beam.ParDo(readfromssql(),landing_direct_focus_et_email_bounces,'WORK_DIRECT_FOCUS_ET_EMAIL_BOUNCES'))
        landing_df_et_subscriber_attr  = (truncate_tables | 'WORK DIRECT FOCUS ET SUBSCRIBER' >>  beam.ParDo(readfromssql(),landing_direct_focus_et_subscriber_attr ,'WORK_DIRECT_FOCUS_ET_SUBSCRIBER_ATTR'))
  
#         (landing_df_communication | 'DIRECT FOCUS COMMUNICATION' >>  beam.ParDo(runbqsql(),df_communication))
        (landing_df_et_email_bounces | 'DIRECT FOCUS ET EMAIL BOUNCES' >>  beam.ParDo(runbqsql(),df_et_email_bounces))
        (landing_df_et_subscriber_attr | 'DIRECT FOCUS ET SUBSCRIBER' >>  beam.ParDo(runbqsql(),df_et_subscriber_attr))
...
def main(args_config,args_productconfig,args_env,args_connectionprefix,args_incrementaldate,args_inputdate):
    ...
    global jobname
    jobname="load-"+product_config['productname']+'-'+time.strftime("%Y%m%d")
    global inputdate
    inputdate=args_inputdate
    logging.info('Job Name is %s',jobname)
    
    global landing_direct_focus_communication
    global landing_direct_focus_contact_email
    global landing_direct_focus_et_email_bounces
    global landing_direct_focus_et_subscriber_attr
    global df_communication
    global df_contact_email
    global df_et_email_bounces
    global df_et_subscriber_attr
    global delete_data_direct_focus
    
    delete_data_direct_focus = readfileasstring(cwd + '/sql/delete_direct_focus_data.sql')
    
#     landing_direct_focus_communication = readfileasstring(cwd + '/sql/work_direct_focus_communication.sql').replace('v_incr_date',incrementaldate).replace('v_inputdate',inputdate)
    landing_direct_focus_et_email_bounces = readfileasstring(cwd + '/sql/work_direct_focus_et_email_bounces.sql').replace('v_incr_date',incrementaldate).replace('v_inputdate',inputdate)
    landing_direct_focus_et_subscriber_attr = readfileasstring(cwd + '/sql/work_direct_focus_et_subscriber_attr.sql').replace('v_incr_date',incrementaldate).replace('v_inputdate',inputdate)

#     df_communication = readfileasstring(cwd + '/sql/df_communication.sql').replace('jobrunid',str(jobrunid)).replace('jobname', jobname)
    df_et_email_bounces = readfileasstring(cwd + '/sql/df_et_email_bounces.sql').replace('jobrunid',str(jobrunid)).replace('jobname', jobname)
    df_et_subscriber_attr = readfileasstring(cwd + '/sql/df_et_subscriber_attr.sql').replace('jobrunid',str(jobrunid)).replace('jobname', jobname)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--inputdate',
        required=True,
        help= ('execution date from ariflow -1 day')
        )
