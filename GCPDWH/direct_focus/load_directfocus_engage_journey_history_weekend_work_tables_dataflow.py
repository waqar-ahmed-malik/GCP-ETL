...
def run():
    ...
    try:
        ...
        (pcoll | 'Initializing..' >> beam.Create(['1'])
        | 'Setting up Instance..' >> beam.ParDo(setenv())
        
        
        | 'WORK ENGAGE JOURNEY HISTORY' >>  beam.ParDo(readfromssql(),work_direct_focus_engage_journey_history,landingtablename)
      
        | 'DIRECTFOCUS ENGAGE JOURNEY HISTORY' >>  beam.ParDo(runbqsql(),direct_focus_engage_journey_history))
...
def main(args_config,args_productconfig,args_env,args_connectionprefix,args_incrementaldate,args_partition,args_landingtablename):
    ...
    global partition
    partition=args_partition
    global landingtablename
    landingtablename=args_landingtablename
    global sqlstring
    global jobrunid
    jobrunid=os.getpid()
    logging.info('Job Run Id is %d',jobrunid)
    global jobname
    jobname="load-engage-journey-history-full-refresh-"+product_config['productname']+'-'+partition+'-'+time.strftime("%Y%m%d")
    logging.info('Job Name is %s',jobname)
        
    global work_direct_focus_engage_journey_history
    global direct_focus_engage_journey_history
        
    work_direct_focus_engage_journey_history = readfileasstring(cwd + '/sql/Weekend/insert_work_df_engage_journey_history_weekend.sql').replace('v_modvalue',partition).replace('v_incr_date',incrementaldate)

    if partition=='0':
		direct_focus_engage_journey_history = readfileasstring(cwd + '/sql/Weekend/insert_direct_focus_engage_journey_history_0.sql').replace('jobrunid',str(jobrunid))
    elif partition=='1':
		direct_focus_engage_journey_history = readfileasstring(cwd + '/sql/Weekend/insert_direct_focus_engage_journey_history_1.sql').replace('jobrunid',str(jobrunid))
    elif partition=='2':
		direct_focus_engage_journey_history = readfileasstring(cwd + '/sql/Weekend/insert_direct_focus_engage_journey_history_2.sql').replace('jobrunid',str(jobrunid))
    elif partition=='3':
		direct_focus_engage_journey_history = readfileasstring(cwd + '/sql/Weekend/insert_direct_focus_engage_journey_history_3.sql').replace('jobrunid',str(jobrunid))
    elif partition=='4':
		direct_focus_engage_journey_history = readfileasstring(cwd + '/sql/Weekend/insert_direct_focus_engage_journey_history_4.sql').replace('jobrunid',str(jobrunid))
    elif partition=='5':
		direct_focus_engage_journey_history = readfileasstring(cwd + '/sql/Weekend/insert_direct_focus_engage_journey_history_5.sql').replace('jobrunid',str(jobrunid))
