...
def run():
    ...
    try:
        ...
        | 'ROW DATE AND FALG UPDATE' >>  beam.ParDo(runbqsql(),direct_focus_individual_dim_flag_update)
        |'WORK MDM MONTHLY LEXISNEXIS_INDIVIDUAL' >>  beam.ParDo(runbqsqloop(),load_mdm_stg_sql) 
...
def main(args_config,args_productconfig,args_env,args_connectionprefix,args_incrementaldate):
    ...
    global jobname
    jobname="load-individual-dim-table"+product_config['productname']+'-'+time.strftime("%Y%m%d")
    ...
    load_mdm_stg_sql=readfileasstring(mdmpath+"/lexisnexis/load_mdm_stg_full.sql")
    load_mdm_processing_sql=readfileasstring(mdmpath+"/lexisnexis/mdm_processing_monthly.sql")
